...
// Get returns one user identified by id
...
// Delete deletes a user record from the database, by id
...
// Insert creates a new user record in the database and returns the newly inserted id
...
// ResetPassword updates a user's password, by id, using the supplied password
...
// It returns true if valid, and false if the password does not match, or if there is an error.
// Note that an error is only returned if something goes wrong (since an invalid password is not
// an error -- it's just the wrong password)
